...
class Agent(BaseAgent):
    ...
    def agent_start(self, observation):
        ...
        # Initiate the portfolio, buy for each crypto 1/5 of the total money amount
        last_values = self.info_market[-1,-4:] # get the last market value 
        self.portfolio = {}
        for i , name in enumerate(self.crypto_name):
            self.portfolio[name] = [((self.cash/5) / last_values[i], last_values[i])]  # (num crypto , purchase price)

        self.cash = self.cash/5 #update cash
        
        self.action = [0,0,0,0] # hold everything

        return self.portfolio , self.action, self.cash/5
    ...
    def get_possible_portfolio(self,crypto,current):
        ...
        if self.cash !=0: # enough money to buy
            buy_portfolio = self.portfolio[crypto].copy() + [((self.cash/4)/current,current)]
        else:
    ...
    def agent_step(self, reward,observation):
        ...
        result = np.array(self.obs.copy()+market_values)
        self.info_market = np.vstack((self.info_market,result)) # update the info on the larket with trye value guven by env

        ############ Prediction step ##########
        X = np.copy(self.info_market[:,:-4])
        y = self.info_market[:,-4:]
